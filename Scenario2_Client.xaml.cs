...
namespace SDKTemplate
{
    ...
    public sealed partial class Scenario2_Client : Page
    {
       ...
       private Polyline polyline = null;
        ...
        public int GetDeviceVoiceLanguage(string langStr)
        {
            int langCode;
            switch (langStr)
            {
        ...
        public string GetDeviceVoiceLanguage(int langCode)
        {
            switch (langCode)
            {
                case 1:
        ...
        private void Volume_Changed()
        {
            ...
            else
            {
                if (sdkLib.GetStatus() == eBeamSDKLib.PEN_DATA_READY &&
                    ConnectionStatus.Text.CompareTo("DATA READY") == 0)
                {
                    if (SoundSel == null) return;
                    if (VolumeSlider == null) return;
                    int audioMode = SoundSel.SelectedIndex;
                    int vol = (int)VolumeSlider.Value;
                    sdkLib.setAudio(audioMode, vol);
