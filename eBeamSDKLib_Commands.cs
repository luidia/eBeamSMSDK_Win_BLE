...
namespace SDKTemplate
{
    ...
    public partial class eBeamSDKLib
    {
        ...
        private async Task<bool> cmdSetAudio(int mode, int vol)
        {
            byte[] data = new byte[] { 0x00, 0x90, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF };
            data[3] = (byte)mode;
            data[4] = (byte)vol;
            for (int i = 1; i < 9; i++) data[0] += data[i];
            var ret = await WriteValue("cmdSetAudio", data);
            return ret;
        }
        private async Task<bool> cmdSetAudioLang(int lang)
        {
            byte[] data = new byte[] { 0x00, 0x90, 0x87, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF };
            data[3] = (byte)lang;
            for (int i = 1; i < 9; i++) data[0] += data[i];
            var ret = await WriteValue("cmdSetAudioLang", data);
            return ret;
        }
        private async Task<bool> cmdSetPosition(int pos)
        {
            byte[] data = new byte[] { 0x00, 0x90, 0x82, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF };
            data[3] = (byte)pos;
            for (int i = 1; i < 9; i++) data[0] += data[i];
            var ret = await WriteValue("cmdSetPosition", data);
            return ret;
        }
    }
}
